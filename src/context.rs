...
impl Context {
    ...
    pub fn symbol_gil<'py, 'a: 'py, T: ToString>(&'a self, py: Python<'py>, name: T) -> PyResult<SymbolGIL<'py>> {
        SymbolGIL::new(py, self, name)
    }
    pub fn symbol_non_commutative<T: ToString>(&self, name: T) -> PyResult<Symbol> {
        Python::with_gil(|py| self.symbol_non_commutative_gil(py, name).map(|a| a.un_gil()))
    }
    pub fn symbol_non_commutative_gil<'py, 'a: 'py, T: ToString>(&'a self, py: Python<'py>, name: T) -> PyResult<SymbolGIL<'py>> {
        SymbolGIL::new(py, self, name)
    }
